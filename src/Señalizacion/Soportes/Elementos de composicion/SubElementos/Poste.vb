...
Public Class Poste
    ...
    'Atributos
    Shared ReadOnly ESQUEMA_XML As String = My.Resources.XML_SCHEME_POSTE

    'Constructores
    Public Sub New()
        Dim ahora = Now
        With Now
            Me.identificador = "poste" & .Year & .Month & .Day & .Hour & .Minute & .Second & .Millisecond
        End With
        Me.anchura = 0.0
        Me.altura = 0.0
        Me.grosor = 0.0
    ...
    Public Sub New(ByVal anchura As Double, _
                   ...
                   ByVal posicion As Punto3D, _
                         Optional ByVal textura As String = Poste.DEFAULT_TEXTURA)
        Dim ahora = Now
        With Now
            Me.identificador = "poste" & .Year & .Month & .Day & .Hour & .Minute & .Second & .Millisecond
        End With
    ...
    Public Overrides Sub GenerarDAE(ByVal PathDestino As String)
        Dim XML As String = Poste.ESQUEMA_XML
        ...
        End With
        XML = XML.Replace("*ANCHURA/2*", (Me.anchura / 2).ToString.Replace(",", "."))
        XML = XML.Replace("*ALTURA*", Me.altura.ToString.Replace(",", "."))
        XML = XML.Replace("*GROSOR/2*", (Me.grosor / 2).ToString.Replace(",", "."))
